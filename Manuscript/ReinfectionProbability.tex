...
\par Shorter immunity periods ({\it e.g.,} $\alpha_R\in\{0.2,0.5}$) allow the individual to get infected several times, which translates into the maximum for the curve corresponding to $M=0$ occurring closer to $R_0=1$ and decreasing faster after this value. It is also worth to point out that for large values of $R_0$ all the curves for $M\in\{1,2,3\}$ join before reaching values near $0$, which means that for large values of $R_0$, the individual can suffer exactly $1$, exactly $2$ or exactly $3$ infections with equal probability.
\begin{figure}[h!]
  \centering
