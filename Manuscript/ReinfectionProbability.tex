...
\begin{itemize}
    ...
    %\item In Figure \ref{fig:siris} we consider the TPI-SIR model for $\alpha_I=0$, where both partial and temporary immunity are present. We consider $\sigma\in\{0.1,0.2,0.5\}$ and $\alpha_R\in\{0.1,0.2,0.5\}$. Since we have here several parameters to vary, we focus only on probabilities $p_{(N-1,S)}^M-p_{(N-1,S)}^{M+1}$ for $M\in\{1,2\}$.
\end{itemize}
...
=======
 \caption{TPI-SIR Model with $\alpha_I=\sigma=0$, accounting only for temporary immunity ($\alpha_R>0$). Probability $p_{(N-1,S)}^M-p_{(N-1,S)}^{M+1}$ of individual $A$, initially susceptible, suffering exactly $M$ infections, for $M\in\{0,1,2,3\}$, versus different values of $R_0=\beta N$. $\gamma=1$ and $N=100$. Immunity is lost at rate
>>>>>>> origin/master
...
=======
 \caption{TPI-SIR Model with $\alpha_I=\alpha_R=0$, accounting only for partial immunity ($\sigma>0$). Probability $p_{(N-1,S)}^M-p_{(N-1,S)}^{M+1}$ of individual $A$, initially susceptible, suffering exactly $M$ infections, for $M\in\{0,1,2,3\}$, versus different values of $R_0=\beta N$. $\gamma=1$ and $N=100$.
>>>>>>> origin/master
...
%\begin{figure}[h!]
%  \centering
% \includegraphics[width=0.6\textwidth]{SIRIS.jpg}
% \caption{TPI-SIR Model with $\alpha_I=0$. Probability $p_{(N-1,S)}^M-p_{(N-1,S)}^{M+1}$ of individual $A$, initially susceptible, suffering exactly $M$
%infections, for $M\in\{1,2\}$, versus different values of $R_0=\beta N$. $\gamma=1$ and $N=100$. Partial immunity is represented here by parameter $\sigma=0.1$ ({\it top-left}), $\sigma=0.2$ ({\it %bottom-left}), $\sigma=0.5$ ({\it top-right}) and
%$\sigma=1.0$ ({\it bottom-right}). Temporary immunity is given by $\alpha_R\in\{0.1,0.2,0.5\}$ ({\it black}, {\it blue} and {\it red} curves, respectively).}
% \label{fig:siris}
%\end{figure}
